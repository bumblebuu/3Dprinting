...
export class DataService {
  ...
  readDocument(collectionName: string, query?: any) {
    if (query) {
      this.http
        .get(`${this.apiURL}/${collectionName}/${query}`)
        .forEach((data) => {
    ...
    this.http.get(`${this.apiURL}/${collectionName}`).forEach((data) => {
      if (collectionName == "products") {
  ...
  readActiveProducts(collectionName: string) {
    this.http
      .get(`${this.apiURL}/${collectionName}/active/ones`)
      .forEach((data) => this.productList.next(data));
  }
  ...
  updateDocument(
    ...
    return this.http.put(
      `${this.apiURL}/${collectionName}/update/${query}`,
      data
    );
  }
  ...
  deleteDocument(collectionName: string, query: any): void {
    this.http
      .delete(`${this.apiURL}/${collectionName}/delete/${query}`)
      .forEach((done) => this.readDocument(collectionName));
  }
}
